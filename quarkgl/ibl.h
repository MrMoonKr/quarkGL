...
public:
    GGXPrefilteredEnvMapCalculator( int width, int height,
                                    int maxNumMips = -1 );
    explicit GGXPrefilteredEnvMapCalculator( ImageSize size,
                                             int maxNumMips = -1 )
        : GGXPrefilteredEnvMapCalculator( size.width, size.height,
                                          maxNumMips ) {}
    virtual ~GGXPrefilteredEnvMapCalculator() = default;

    unsigned int getNumSamples() const { return shader_.getNumSamples(); }
    void setNumSamples( unsigned int samples ) {
        shader_.setNumSamples( samples );
    }

    // Draw onto the allocated prefiltering cubemap from the given cubemap as
    // the source. The cubemap should ideally have mip levels in order to avoid
    // hotspot artifacts.
    void multipassDraw( Texture source );
...
public:
    ...
    // Draw onto the allocated BRDF integration texture. This is solely a
    // function of the BRDF and does not require any source data.
    void draw();
