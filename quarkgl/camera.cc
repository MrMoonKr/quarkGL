...
void Camera::lookAt( glm::vec3 center ) {
    ...
    yaw_ = glm::mod<float>( glm::degrees( atan2( dir.x, dir.z ) ) * -1.0f,
                            360.0f ) +
           90.0f;
    updateCameraVectors();
}
...
glm::mat4 Camera::getProjectionTransform() const {
    return glm::perspective( glm::radians( getFov() ), aspectRatio_, near_,
                             far_ );
}
...
OrbitCameraControls::OrbitCameraControls( Camera& camera, glm::vec3 center )
    ...
    azimuth_ =
        glm::mod<float>( glm::degrees( atan2( normdir.x, normdir.z ) ) * -1.0f,
                         360.0f ) +
        90.0f;
    altitude_ = glm::clamp<float>( glm::degrees( asin( normdir.y ) ),
                                   -POLAR_CAP, POLAR_CAP );
...
void OrbitCameraControls::scroll( Camera& camera, double xoffset,
                                  double yoffset, bool mouseCaptured ) {
    radius_ =
        glm::clamp( radius_ - static_cast<float>( yoffset * sensitivity_ ),
...
void OrbitCameraControls::mouseMove( Camera& camera, double xpos, double ypos,
    ...
    altitude_ = glm::clamp( altitude_ - delta.yoffset * sensitivity_,
                            -POLAR_CAP, POLAR_CAP );
