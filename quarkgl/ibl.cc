...
CubemapIrradianceShader::CubemapIrradianceShader()
    : Shader(
          ShaderPath( "quarkgl/shaders/builtin/cubemap.vert" ),
...
CubemapIrradianceCalculator::CubemapIrradianceCalculator( int width,
                                                          int height )
...
qrk::GGXPrefilteredEnvMapCalculator::GGXPrefilteredEnvMapCalculator(
    ...
    TextureParams textureParams = {
        ...
        // Force mip generation. We can optionally just use a smaller part of
        // the
        // mip chain.
...
GGXBrdfIntegrationCalculator::GGXBrdfIntegrationCalculator( int width,
    ...
    // The BRDF integration map contains values from [0..1] so we can use an
    // SNORM for greater precision.
