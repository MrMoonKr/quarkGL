...
Texture Attachment::asTexture() {
    if ( target != AttachmentTarget::TEXTURE ) {
        throw FramebufferException(
            "ERROR::FRAMEBUFFER::INVALID_ATTACHMENT_TARGET" );
    }
    Texture texture;
    texture.id_ = id;
        ...
        switch ( attachment.target ) {
            ...
            case AttachmentTarget::RENDERBUFFER:
                ...
                if ( cubemapFace >= 0 ) {
                    ...
                    throw FramebufferException(
                        "ERROR::FRAMEBUFFER::CUBEMAP_FACE_GIVEN_FOR_"
                        "RENDERBUFFER" );
                }
                break;
        }
...
Attachment Framebuffer::attachTexture( BufferType type,
    ...
    if ( samples_ && textureType != TextureType::CUBEMAP ) {
        glTexStorage2DMultisample( textureTarget, samples_, internalFormat,
                                   width_, height_,
                                   /*fixedsamplelocations=*/GL_TRUE );
    } else {
        glTexStorage2D( textureTarget, numMips, internalFormat, width_,
                        height_ );
    }
...
Attachment Framebuffer::attachRenderbuffer( BufferType type ) {
    ...
    if ( samples_ ) {
        ...
        glRenderbufferStorage( GL_RENDERBUFFER, internalFormat, width_,
                               height_ );
    }
    ...
    if ( glCheckFramebufferStatus( GL_FRAMEBUFFER ) !=
         GL_FRAMEBUFFER_COMPLETE ) {
        throw FramebufferException(
            "ERROR::FRAMEBUFFER::RENDERBUFFER::INCOMPLETE" );
    }
