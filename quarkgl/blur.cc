...
PingPongPass::PingPongPass( int width, int height )
    ...
    attachments_[ 0 ] =
        buffers_[ 0 ].attachTexture( BufferType::COLOR_HDR_ALPHA );
    attachments_[ 1 ] =
        buffers_[ 1 ].attachTexture( BufferType::COLOR_HDR_ALPHA );
}
...
GaussianBlurShader::GaussianBlurShader()
    : ScreenShader(
          ShaderPath( "quarkgl/shaders/builtin/gaussian_blur.frag" ) ) {}
