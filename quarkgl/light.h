...
public:
    ...
    // Applies the view transform to the registered lights. This is
    // automatically called if a view source has been set.
    void applyViewTransform( const glm::mat4& view );

    // Sets whether the registered lights should transform their positions to
    // view space. If false, positions remain in world space when passed to the
    // shader.
    void setUseViewTransform( bool useViewTransform );
