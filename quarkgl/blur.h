...
public:
    ...
    // first and then ping-ponging between internal framebuffers. Passes
    // represent a single ping-pong cycle (so N passes means N*2 draw calls).
    // The callback should configure or update any needed shader uniforms, etc.
    // After drawing, you can call getOutput() to get the resulting texture.
