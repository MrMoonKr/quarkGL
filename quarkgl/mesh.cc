...
void Mesh::initializeVertexArrayInstanceData() {
    if ( instanceCount_ ) {
        ...
        vertexArray_.allocateInstanceVertexData( instanceCount_ *
                                                 sizeof( glm::mat4 ) );
        // Add vertex attributes (max attribute size is vec4, so we need 4 of
        // them).
        ...
        if ( type == TextureMapType::CUBEMAP ) {
            ...
            switch ( type ) {
                ...
                case TextureMapType::AO:
                    ss << "aoMaps[" << aoIdx << "]";
                    shader.setBool( materialName + ".aoIsPacked[" +
                                        std::to_string( aoIdx ) + "]",
                                    textureMap.isPacked() );
                    aoIdx++;
                    break;
...
void Mesh::glDraw() {
    ...
    if ( instanceCount_ ) {
        ...
        if ( !indices_.empty() ) {
            glDrawElementsInstanced( GL_TRIANGLES, indices_.size(),
                                     GL_UNSIGNED_INT, nullptr, instanceCount_ );
        } else {
            glDrawArraysInstanced( GL_TRIANGLES, 0, numVertices_,
                                   instanceCount_ );
        }
