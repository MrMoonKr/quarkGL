...
void CubeMesh::loadMeshAndTextures(
    const std::vector<TextureMap>& textureMaps ) {
...
void RoomMesh::loadMeshAndTextures(
    const std::vector<TextureMap>& textureMaps ) {
...
void SphereMesh::loadMeshAndTextures(
    const std::vector<TextureMap>& textureMaps ) {
    // Generate the sphere vertex components. This uses the common "UV"
    // approach.
    ...
    for ( unsigned int iy = 0; iy <= heightSegments; ++iy ) {
        ...
        // However, we adjust it by a tiny amount to ensure that we don't
        // generate tangents of zero length (due to the sin() in the
        // calculations).
        ...
        for ( unsigned int ix = 0; ix <= widthSegments; ++ix ) {
            ...
            vertexData.push_back(
                0.0f );  // Spherical tangents ignore parallels.
            vertexData.push_back( x );
            // Texture coordinates.
            vertexData.push_back( u + uOffset );
            vertexData.push_back(
                1.0f -
                v );  // We're generating the sphere top-down, so reverse V.
        }
    }

    std::vector<unsigned int> indices;
    // Since we created an extra duplicate "wraparound" vertex for each
    // parallel, we have to adjust the stride.
...
void SkyboxMesh::loadMesh() {
    ...
    loadMeshData( skyboxVertices,
                  sizeof( skyboxVertices ) / skyboxVertexSizeBytes,
