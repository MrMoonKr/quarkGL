...
Texture Texture::load( const char* path, bool isSRGB,
    ...
    if ( texture.numChannels_ == 1 ) {
        ...
        throw TextureException(
            ...
            "Texture '" +
            std::string( path ) +
            "' contained unsupported number of channels: " +
...
Texture Texture::createCubemap( int size, GLenum internalFormat,
    ...
    glTexStorage2D( GL_TEXTURE_CUBE_MAP, texture.numMips_,
                    texture.internalFormat_, texture.width_, texture.height_ );
...
void Texture::bindToUnit( unsigned int textureUnit, TextureBindType bindType ) {
    ...
    switch ( bindType ) {
        ...
        case TextureBindType::IMAGE_TEXTURE:
            // Bind image unit.
            glBindImageTexture( textureUnit, id_, /*level=*/0,
                                /*layered=*/GL_FALSE, 0, GL_READ_WRITE,
                                internalFormat_ );
            break;
        default:
            throw TextureException(
                "ERROR::TEXTURE::INVALID_TEXTURE_BIND_TYPE\n" +
