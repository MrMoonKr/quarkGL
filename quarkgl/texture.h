...
public:
    ...
    // Generates mipmaps for the current texture. Note that this will not
    // succeed for textures with immutable storage.
...
private:
    // TODO: Texture lifetimes aren't managed currently, so they aren't
    // unloaded.
    unsigned int id_;
    TextureType type_;
    std::string path_;
