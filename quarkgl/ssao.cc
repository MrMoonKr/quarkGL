...
void SsaoKernel::regenerate( int kernelSize, int noiseTextureSideLength ) {
    ...
    for ( int i = 0; i < kernelSize; ++i ) {
        ...
        glm::vec3 sample( rand_.next() * 2.0f - 1.0f,
                          rand_.next() * 2.0f - 1.0f, rand_.next() );
        // Reject samples outside the sphere, to avoid over-sampling the
        // corners.
        ...
        // At this point we have a random point sampled in the hemisphere, but
        // we want to sample more points closer to the actual fragment, so we
        // scale the result.
    ...
    noiseTexture_ = Texture::createFromData(
        noiseTextureSideLength, noiseTextureSideLength,
...
void SsaoKernel::updateUniforms( Shader& shader ) {
    ...
    for ( unsigned int i = 0; i < kernel_.size(); ++i ) {
        shader.setVec3( "qrk_ssaoKernel[" + std::to_string( i ) + "]",
                        kernel_[ i ] );
    }
}
...
void SsaoBlurShader::configureWith( SsaoKernel& kernel, SsaoBuffer& buffer ) {
    setInt( "qrk_ssaoNoiseTextureSideLength",
            kernel.getNoiseTextureSideLength() );
