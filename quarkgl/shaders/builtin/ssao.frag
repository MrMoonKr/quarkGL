...
void main() {
    ivec2 noiseSize = textureSize(qrk_ssaoNoise, /*u_lod=*/0);
    ...
    mat3 fragTBN_viewSpace =
            qrk_calculate_tbn(fragNormal_viewSpace, noise_tangentSpace);

    // Calculate the occlusion factor.
    float occlusion = 0.0;
    for (int i = 0; i < qrk_ssaoKernelSize; ++i) {
        // Calculate sample position. Convert from tangent to u_view space.
        ...
        vec4 samplePos_clipSpace = u_projection * vec4(samplePos_viewSpace, 1.0);
        ...
        float sampleDepth = texture(u_gPositionAO, samplePos_clipSpace.xy).z;
