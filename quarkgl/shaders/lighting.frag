...
vec3 qrk_extract_albedo(QrkMaterial u_material, vec2 texCoords) {
...
vec3 qrk_extract_specular(QrkMaterial u_material, vec2 texCoords) {
...
float qrk_extract_roughness(QrkMaterial u_material, vec2 texCoords) {
...
float qrk_extract_metallic(QrkMaterial u_material, vec2 texCoords) {
 ...
 * all occluded.
 */
float qrk_extract_ambient_occlusion(QrkMaterial u_material, vec2 texCoords) {
    float ao = 1.0;
...
vec3 qrk_extract_emission(QrkMaterial u_material, vec2 texCoords) {
 ...
 */
float qrk_material_alpha(QrkMaterial u_material, vec2 texCoords) {
    float sum = 0.0;
 ...
 */
float qrk_calc_attenuation(QrkAttenuation attenuation, float fragDist) {
 ...
 * a fragment.
 */
float qrk_calc_spot_light_intensity(QrkSpotLight light, vec3 lightDir) {
...
vec3 qrk_shade_ambient_deferred(vec3 albedo, vec3 u_ambient, float ao) {
    return albedo * u_ambient * ao;
}

/** Calculate shading for u_ambient component based on the given u_material. */
vec3 qrk_shade_ambient(QrkMaterial u_material, vec3 albedo, vec2 texCoords) {
    float ao = qrk_extract_ambient_occlusion(u_material, texCoords);
    return qrk_shade_ambient_deferred(albedo, u_material.u_ambient, ao);
}

/** ============================ Emission ============================ **/

/** Calculate deferred shading for emission based on an emission color. */
vec3 qrk_shade_emission_deferred(vec3 emissionColor, vec3 fragPos_viewSpace,
                               QrkAttenuation u_emissionAttenuation) {
    // Calculate emission attenuation towards camera.
    float fragDist = length(fragPos_viewSpace);
    float attenuation = qrk_calc_attenuation(u_emissionAttenuation, fragDist);
    // Emission component.
    return emissionColor * attenuation;
}

/** Calculate shading for emission textures on the given u_material. */
vec3 qrk_shade_emission(QrkMaterial u_material, vec3 fragPos_viewSpace,
                       vec2 texCoords) {
    vec3 emission = qrk_extract_emission(u_material, texCoords);
    return qrk_shade_emission_deferred(emission, fragPos_viewSpace,
 ...
 */
vec3 qrk_get_normal(QrkMaterial u_material, vec2 texCoords, mat3 TBN,
                   vec3 vertexNormal) {
    if (u_material.hasNormalMap) {
        return normalize(TBN * qrk_sample_normal_map(u_material.u_normalMap, texCoords));
    } else {
...
float qrk_shadow_bias(float minBias, float maxBias, vec3 normal, vec3 lightDir) {
    return max(maxBias * (1.0 - dot(normal, lightDir)), minBias);
}

/** Sample from a shadow map using 9-texel percentage-closer filtering. */
float qrk_shadow_sample_pcf(sampler2D u_shadowMap, vec2 shadowTexCoords,
...
float qrk_shadow(sampler2D u_shadowMap, vec4 fragPosLightSpace, float bias) {
    ...
    return qrk_shadow_sample_pcf(u_shadowMap, shadowTexCoords, currentDepth, bias);
}
