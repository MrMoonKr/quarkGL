...
ModelMesh::ModelMesh( const std::vector<ModelVertex>& vertices,
    ...
    : vertices_( vertices ) {
    loadMeshData( &vertices_[ 0 ], vertices_.size(), sizeof( ModelVertex ),
                  indices, textureMaps, instanceCount );
}
...
Model::Model( const char* path, unsigned int instanceCount )
    ...
    // This will either be the model's directory, or empty string if the model
    // is at project root.
...
std::vector<TextureMap> Model::loadMaterialTextureMaps( aiMaterial* material,
    ...
    for ( aiTextureType aiType : aiTypes ) {
        for ( unsigned int i = 0; i < material->GetTextureCount( aiType );
              i++ ) {
            ...
            if ( item != loadedTextureMaps_.end() ) {
                // Texture has already been loaded, but likely of a different
                // map type (for example, it could be a combined roughness /
                // metallic map). If so, mark it as a packed texture.
