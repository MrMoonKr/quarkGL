...
int main() {
    ...
    qrk::Shader pbrShader(
        qrk::ShaderPath( "examples/shaders/normal_map.vert" ),
    ...
    qrk::TextureMap rustedironNormalMap(
        qrk::Texture::load(
            "examples/assets/rusted_iron/rustediron2_normal.png",
            /*isSRGB=*/false ),
        qrk::TextureMapType::NORMAL );
    qrk::TextureMap rustedironSpecularMap(
        qrk::Texture::load(
            "examples/assets/rusted_iron/rustediron2_metallic.png",
            /*isSRGB=*/false ),
    ...
    printf(
        "Rendering %dx%d grid of spheres with varying roughness (X) and "
        "metallic "
        "(Y)\n",
    ...
    win.loop( [ & ]( float deltaTime ) {
        ...
        for ( int r = 0; r < NUM_SPHERE_ROWS; ++r ) {
            ...
            for ( int c = 0; c < NUM_SPHERE_COLS; ++c ) {
                ...
                float xOffset =
                    ( c - ( NUM_SPHERE_COLS / 2 ) ) * SPHERE_SPACING;
                float yOffset =
                    ( r - ( NUM_SPHERE_ROWS / 2 ) ) * SPHERE_SPACING;
